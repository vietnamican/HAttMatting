...
class Model(nn.Module):
    ...
    def forward(self, x):
        ...
        # Stage 5d
        x5t = torch.add(F.max_unpool2d(self.trimap_deconv6_1(x61), id5, kernel_size=2, stride=2), x53)
        # x5t = x5t + x53
        # x51t = self.trimap_deconv5_1(x5t)
        # Stage 4d
        x4t = torch.add(F.max_unpool2d(self.trimap_deconv5_1(x5t), id4, kernel_size=2, stride=2), x43)
        # x4t = x4t + x43
        # x41t = self.trimap_deconv4_1(x4t)
        # Stage 3d
        x3t = torch.add(F.max_unpool2d(self.trimap_deconv4_1(x4t), id3, kernel_size=2, stride=2), x33)
        # x3t = x3t + x33
        # x31t = self.trimap_deconv3_1(x3t)
        # Stage 2d
        x2t = torch.add(F.max_unpool2d(self.trimap_deconv3_1(x3t), id2, kernel_size=2, stride=2), x22)
        # x2t = x2t + x22
